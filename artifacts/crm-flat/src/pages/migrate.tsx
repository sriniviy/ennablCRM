...
export function MigratePage() {
    ...
    (async () => {
      try {
        ...
        const [teamRes, stageRes, cfRes] = await Promise.all([
          fetch(`/api/team`, { headers: headersInit }),
          fetch(`/api/migrate/stages`, { headers: headersInit }),
          fetch(`/api/custom-fields`, { headers: headersInit }),
        ]);
        if (teamRes.ok) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      setProgress(40);
      const res = await fetch(`/api/migrate/${step}`, {
        method: "POST",
