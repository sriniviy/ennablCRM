  ...
  try {
    ...
    const [cfDefs, cfValues] = await Promise.all([
      db.select().from(customFieldDefinitionsTable)
        .where(eq(customFieldDefinitionsTable.objectType, "contact"))
        .orderBy(asc(customFieldDefinitionsTable.displayOrder)),
      rows.length > 0
        ? db.select().from(customFieldValuesTable)
            .where(and(
              eq(customFieldValuesTable.objectType, "contact"),
              inArray(customFieldValuesTable.recordId, rows.map(r => r.contact.id)),
            ))
        : Promise.resolve([]),
    ]);
